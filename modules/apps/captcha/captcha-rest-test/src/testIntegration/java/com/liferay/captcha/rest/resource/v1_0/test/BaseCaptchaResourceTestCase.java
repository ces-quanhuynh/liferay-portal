...
import com.liferay.captcha.rest.client.dto.v1_0.FormCaptcha;
...
public abstract class BaseCaptchaResourceTestCase {
	...
	public void testEscapeRegexInStringFields() throws Exception {
		...
		Assert.assertEquals(regex, captcha.getCaptchaToken());
	...
	@Test
	public void testValidateSimpleCaptcha() throws Exception {
		Captcha randomCaptcha = randomCaptcha();

		Captcha postCaptcha = testValidateSimpleCaptcha_addCaptcha(
			randomCaptcha);

		assertEquals(randomCaptcha, postCaptcha);
		assertValid(postCaptcha);
	}

	protected Captcha testValidateSimpleCaptcha_addCaptcha(Captcha captcha)
		throws Exception {
	...
	protected void assertEquals(
		FormCaptcha formCaptcha1, FormCaptcha formCaptcha2) {

		Assert.assertTrue(
			formCaptcha1 + " does not equal " + formCaptcha2,
			equals(formCaptcha1, formCaptcha2));
	}
	...
	protected void assertValid(Captcha captcha) throws Exception {
		...
		for (String additionalAssertFieldName :
			...
			if (Objects.equals("captchaToken", additionalAssertFieldName)) {
				if (captcha.getCaptchaToken() == null) {
					valid = false;
				}
	...
	protected void assertValid(FormCaptcha formCaptcha) {
		boolean valid = true;

		for (String additionalAssertFieldName :
				getAdditionalFormCaptchaAssertFieldNames()) {

			if (Objects.equals("answer", additionalAssertFieldName)) {
				if (formCaptcha.getAnswer() == null) {
					valid = false;
				}
			...
			if (Objects.equals("captchaToken", additionalAssertFieldName)) {
				if (formCaptcha.getCaptchaToken() == null) {
					valid = false;
				}
	...
	protected String[] getAdditionalFormCaptchaAssertFieldNames() {
		return new String[0];
	}
		...
		for (String additionalAssertFieldName :
			...
			if (Objects.equals("captchaToken", additionalAssertFieldName)) {
				if (!Objects.deepEquals(
						captcha1.getCaptchaToken(),
						captcha2.getCaptchaToken())) {
	...
	protected boolean equals(
		...
		for (String additionalAssertFieldName :
			...
			if (Objects.equals("answer", additionalAssertFieldName)) {
				if (!Objects.deepEquals(
						formCaptcha1.getAnswer(), formCaptcha2.getAnswer())) {
			...
			if (Objects.equals("captchaToken", additionalAssertFieldName)) {
				if (!Objects.deepEquals(
						formCaptcha1.getCaptchaToken(),
						formCaptcha2.getCaptchaToken())) {
	...
	protected String getFilterString(
		...
		if (entityFieldName.equals("captchaToken")) {
			Object object = captcha.getCaptchaToken();
	...
	protected Captcha randomCaptcha() throws Exception {
		return new Captcha() {
			{
				captchaToken = StringUtil.toLowerCase(
	...
	protected FormCaptcha randomFormCaptcha() throws Exception {
		return new FormCaptcha() {
			{
				answer = RandomTestUtil.randomString();
				captchaToken = RandomTestUtil.randomString();
			}
		};
	}
